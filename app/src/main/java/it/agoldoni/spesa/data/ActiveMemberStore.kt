...
@Singleton
class ActiveMemberStore @Inject constructor(
    @ApplicationContext context: Context
) {
    private val prefs = context.getSharedPreferences("spesa_prefs", Context.MODE_PRIVATE)

    fun observeActiveMemberId(): Flow<String?> = callbackFlow {
        trySend(get())
    ...
    fun set(memberId: String) {
        prefs.edit().putString(KEY, memberId).apply()
    }
